...
namespace KerbonautRedux
{
    ...
    public class HairConfigs
    {
            ...
            for (int i = 0; i < arrayContent.Length; i++)
            {
                ...
                else if (c == '}')
                {
                    ...
                    if (braceDepth == 0 && objStart >= 0)
                    {
                        ...
                        {
                            pieces.Add(piece);
                            Debug.Log($"[KerbonautRedux] ParseHairPieces: Added piece '{piece.MeshName}'");
                        }
                        objStart = -1;
                        objectCount++;
                    }
        ...
        public Material CreateMaterial(Texture2D diffuseTexture, Texture2D normalMap, Color color)
        {
            Debug.Log($"[KerbonautRedux] Creating material... diffuse is null: {diffuseTexture == null}, normalMap is null: {normalMap == null}");
            
            Material mat;
            bool useTransparency = diffuseTexture != null && HasTransparency(diffuseTexture);
            ...
            if (normalMap != null)
            {
                ...
                Debug.Log($"[KerbonautRedux] *** USING BUMPED SPECULAR SHADER ***");
            }
            else if (useTransparency)
            {

                mat = new Material(Shader.Find("KSP/Alpha/Translucent"));
                Debug.Log($"[KerbonautRedux] *** USING ALPHA TRANSLUCENT SHADER (transparent texture detected) ***");
            }
            else
            {
        ...
        private bool HasTransparency(Texture2D texture)
        {
            ...
            try
            {
                ...
                int checkInterval = Mathf.Max(1, pixels.Length / 100);
                
                for (int i = 0; i < pixels.Length; i += checkInterval)
                {
                    if (pixels[i].a < 255)
                    {
                        return true;
        ...
        public Material CreateMaterial(Texture2D texture, Color color)
        {
            return CreateMaterial(texture, null, color);
        }

        public Material CreateMaterial(string texturePath, Color color)
        {
            Debug.Log($"[KerbonautRedux] CreateMaterial called with texturePath: '{texturePath}'");
            
            Texture2D tex = GetTexture(texturePath);
            return CreateMaterial(tex, null, color);
        }
        
        public Material CreateMaterial(string diffusePath, string normalMapPath, Color color)
        {
            Debug.Log($"[KerbonautRedux] CreateMaterial called with diffuse: '{diffusePath}', normal: '{normalMapPath}'");
            ...
            return CreateMaterial(diffuse, normalMap, color);
        }
